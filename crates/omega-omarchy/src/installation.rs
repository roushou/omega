...
use omega_host::{AtomicFile, GenerationId, Layout};
...
impl ShellInstallation {
    ...
    fn lock(&self) -> Result<omega_host::fs::FileLock, InstallError> {
        std::fs::create_dir_all(self.layout.generations_dir())?;
        Ok(omega_host::fs::FileLock::exclusive(
            &self.layout.shell_lock(),
        )?)
    }
